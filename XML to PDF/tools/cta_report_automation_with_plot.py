...
- Fixes path bug by using absolute XPath from the document root for knee-points.
- Restores a proper <img> tag when injecting the PNG into HTML.
- Makes axes dynamic, shows the 100 µ decade when data < 1 mA, and adds headroom so the plot never touches borders.
"""
...
def _y_minor_formatter_2_5():
    """
    Label Y-axis minor ticks at mantissas 2 and 5 on log scale (e.g., 0.2, 0.5, 2, 5, 20).
    """
    def fmt(y, _pos):
        if y <= 0:
            return ''
...
def build_excitation_plot_png(
    ...
    """
    Builds the excitation plot PNG.
    If kneepoint is provided, draw dashed lines at (Vk, Ik).
    """
    V_vals, I_vals = [], []
    ...
    # Sort by I for loglog curve continuity
    order = np.argsort(I)
    I, V = I[order], V[order]

    # Axis ranges include kneepoint if provided
    ...
    # --- Dynamic limits (with 100 µ decade and headroom) ------------------------
    x_lo, x_hi = _snap_limits_125(xmin_raw, xmax_raw)
    y_lo, y_hi = _snap_limits_125(ymin_raw, ymax_raw)

    # If the data dips below 1 mA, force the lower decade to include 100 µA.
    if xmin_raw < 1e-3:
        x_lo = min(x_lo, 1e-4)

    # Add top headroom so the curve and dashed lines never touch the frame.
    y_hi *= 1.20

    # If a Vk is present and sits near the top, expand again for space.
    if (Vk is not None) and (Vk > 0) and (Vk >= 0.98 * y_hi):
        y_hi = Vk * 1.08

    # Overhang for each line (so each can draw even if the other is missing)
    ...
    # Draw knee-point lines if available
    if Vk is not None and Vk > 0:
        ax.plot([x_lo, Ix_end if Ix_end else x_hi], [Vk, Vk],
                color=REF_COLOR, linewidth=REF_LW, linestyle=REF_DASH)
    if Ik is not None and Ik > 0:
        ax.plot([Ik, Ik], [y_lo, Vy_end if Vy_end else y_hi],
                color=REF_COLOR, linewidth=REF_LW, linestyle=REF_DASH)
...
def extract_excitation_rows_from_xml(xml_path: Path, v_field: str = "UCTrms") -> List[tuple]:
    # Absolute XPath from document root 'Object'
...
def _normalize_standard_name(name: Optional[str]) -> Optional[str]:
    """Map user/CLI/embedded names to the XML node names under KneePoints."""
    if not name:
        return None
...
def _read_standard_from_xml(xml_path: Path) -> Optional[str]:
    """Read and map to a KneePoints section name."""
    try:
        tree = etree.parse(str(xml_path))
        ...
        # When the nameplate standard is 61869-2, we use IEC_69_2 node under KneePoints
...
def get_kneepoint_pair(xml_path: Path, standard: Optional[str] = "IEC_69_2") -> Optional[Tuple[float, float]]:
    """Return (Vk, Ik) knee-point pair from XML or None if missing/invalid."""
    try:
        ...
        # Fallback to IEC_69_2 if the requested std is missing
...
def inject_plot_into_html(html_text: str, img_png_bytes: bytes, target_section_hint: str = 'excitation') -> str:
    ...
    # Primary lookup for placeholder
    ...
    repl_html = lxml_html.fragment_fromstring(f'\n<div style="text-align:center">{img_tag}</div>\n')
...
def process_one_xml(
    ...
    excitation_source: str = "xml",     # where to get V/I rows: "xml" or "html"
    excitation_v: str = "UCTrms",       # UCTrms | UCorerms | UCTrect
    vref_source: str = "xml",           # "xml" -> knee-point from XML; "auto" -> omit lines
    kneepoint_standard: str = "IEC_69_2",
    force_kneepoint: Optional[Tuple[float, float]] = None,  # (Vk, Ik) if provided
    debug_dump_html: bool = False,
    axis_labels: str = 'vendor',
    x_ticks_mode: str = 'minimal',
) -> Tuple[Path, bool, str]:
    """
    Build HTML via XSLT, optionally inject the excitation plot (with XML-provided knee point),
    and convert to PDF.
    """
    try:
        ...
        # Build plot and inject (optional)
                ...
                if rows:
                    ...
                    for attempt in range(2):
                        try:
                            ...
                            if attempt ==1:
                                print(f"[WARN] {xml.pathname}: Plot generation failed after retry : {ex}")
...
def main():
    parser = argparse.ArgumentParser(
        description='CTA XML -> XSLT HTML -> PDF (batch) -> Master PDF (+ Excitation plot with XML knee-point)'
    )
    ...
    # Knee-point controls (default: use knee point from XML, IEC 61869-2)
    parser.add_argument('--vref-source', choices=['xml', 'auto'], default='xml', help='Use knee-point from XML (default) or omit it (auto).')
    parser.add_argument('--kneepoint-standard', choices=['IEC_69_2', 'IEC_1', 'IEC_6', 'ANSI_30', 'ANSI_45'], default='IEC_69_2',
                        help='Which knee-point standard to use when --vref-source=xml (default: IEC_69_2).')
    parser.add_argument('--force-kneepoint', type=_parse_forced_kneepoint, default=None,
                        help='Force a specific knee-point Vk,Ik (e.g., "3.797,0.04609").')

    parser.add_argument('--axis-labels', choices=['vendor', 'classic'], default='vendor',
                        help="'vendor' => X:'I/V', Y:'V/V'; 'classic' => X:'I [A]', Y:'U [V]'.")
    parser.add_argument('--x-ticks', choices=['dense', 'normal', 'minimal'], default='minimal',
                        help='X tick label density on log scale. minimal (default) = label only 1 per decade.')
    ...
    print('[INFO] Found {} XML files. Engine: {}. Converting with {} workers...'
          .format(len(xml_files), args.engine, args.workers))
    ...
    if ok:
        ...
        for p in pdf_order:
            try:
                ...
                print(f"[WARN] could not delete PDF {p}: {e}")
